program linear_interceptor
    ...
    do
        write(*,*) "Welcome to the Linear Interceptor Program."
        write(*,*) "Would you like to read from a file? (0 for no, 1 for yes, 2 to exit program): "
        read *, file_read
        ...
        if (file_read == 0) then
            ...
            do
                ...
                close(10)
                close(11)
                write(*,*) file_name, " successfully written. Would you like to process another file? (0 for no, 1 for yes)"
                read *, file_read
                if(file_read == 0) then
                    exit
                end if
            end do
        else if (file_read == 2) then
            exit
        end if
    end do
    write(*,*) "Thank you for using the Linear Interceptor Program."
